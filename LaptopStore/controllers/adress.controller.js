...
const getAll = (req, res) => {
  ...
  db.query(`select * from adress LIMIT ?`, [limit], (error, result) => {
    if (error) {
...
const getById = (req, res) => {
  ...
  db.query(
    `SELECT * from adress where id=${id};
    `,
    (error, result) => {
      if (error) {
        sendErrorResponse(error, res);
...
const create = (req, res) => {
  ...
  db.query(
    ...
    (error, result) => {
      ...
      res
        .status(201)
        .send({ message: "Yangi adress qoshilid", adressId: result.insertId });
    }
  );
};
...
const update = (req, res) => {
  ...
  db.query(
    ...
    (err, result) => {
      ...
      res.status(200).send({ message: "adress updated successfully" });
    }
  );
};
...
const remove = (req, res) => {
  ...
  db.query(`DELETE FROM adress where id=?`, [id], (err, result) => {
    ...
    res.status(200).send({ message: "adress deleted successfully" });
  });
};
